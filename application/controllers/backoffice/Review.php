    ...
    public function edit()
    {
        ...
        if ($this->input->post('submit_page') == "Submit") {
            ...
            $edit_data = array(
                ...
                'rating'    => $this->input->post('rating') ? $this->input->post('rating')[0] : null,
            );

            if ($this->review_model->updateData($edit_data, 'review', 'entity_id', $entity_id)) {
                $this->session->set_flashdata('page_MSG', $this->lang->line('success_update'));
            }
            redirect(base_url() . ADMIN_URL . '/' . $this->controller_name . '/view');
        }
        $entity_id = ($this->uri->segment('4')) ? $this->encryption->decrypt(str_replace(array('-', '_', '~'), array('+', '/', '='), $this->uri->segment(4))) : $this->input->post('entity_id');
        $data['edit_records'] = $this->review_model->getEditDetail($entity_id);
        $data['res_name']       = $this->review_model->getResName($entity_id)->name;
        $this->load->view(ADMIN_URL . '/review_add', $data);
    }
    //ajax view
    public function ajaxview()
    {
        ...
        foreach ($grid_data['data'] as $key => $val) {
            $records["aaData"][] = array(
                $nCount,
                $val->rname,
                $val->first_name . '<br />' . $val->mobile_number,
                $val->review,
                $val->rating,
                ...
                '<button onclick="deleteDetail(' . $val->entity_id . ')"  title="Click here for delete" class="delete btn btn-sm danger-btn margin-bottom"><i class="fa fa-times"></i> Delete</button>'
                    . '<a class="btn btn-sm danger-btn margin-bottom" title="' . $this->lang->line('edit') . '" href="' . base_url() . ADMIN_URL . '/' . $this->controller_name . '/edit/' . str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encryption->encrypt($val->entity_id)) . '"><i class="fa fa-edit"></i> ' . $this->lang->line('edit') . '</a>'
