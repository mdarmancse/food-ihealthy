...
class Bkash_web extends REST_Controller
{
    ...
    {
        $strJsonFileContents = file_get_contents("https://foodibd.com/bkashconfig_web.json");
    ...
    public function CancelAgreement_post()
    {
        ...
        if ($result['id_token']) {
            ...
            if ($result && $result['statusMessage'] == "Successful") {
                if ($this->deleteAgreement($agreementID)) {
                    $this->response(['status' => 1, 'msg' => $result,], REST_Controller::HTTP_OK);
                } else {
                    $this->response(['status' => 0, 'msg' => "Couldn't complete", 'info' => $result,], REST_Controller::HTTP_OK);
                }
            } else {
                if ($this->deleteAgreement($agreementID)) {
                    $this->response(['status' => 1, 'msg' => $result,], REST_Controller::HTTP_OK);
                ...
                $this->response(['status' => 0, 'msg' => $result,], REST_Controller::HTTP_OK);
            }
        } else {
            if ($this->deleteAgreement($agreementID)) {
                $this->response(['status' => 1, 'msg' => "Agreement not found. Removed from your account.",], REST_Controller::HTTP_OK);
            } else {
                $this->response(['status' => 0, 'msg' => "Couldn't complete", 'info' => $result,], REST_Controller::HTTP_OK);
            }
            $this->response(['status' => -1, 'msg' => "Something Went wrong",], REST_Controller::HTTP_OK);
    ...
    public function deleteAgreement($id)
    {
        ...
        return $this->db->update('agreement_info');
    }
}
