<?php
class Popup_slider_model extends CI_Model {
    ...
    {
        $this->db->select('*');
        return $this->db->get_where('popup_slider_image',array('entity_id'=>$entity_id))->first_row();
    }
    ...
    public function getGridList($sortFieldName = '', $sortOrder = 'ASC', $displayStart = 0, $displayLength = 10)
    {
        ...
        $result['total'] = $this->db->count_all_results('popup_slider_image');
        ...
        $result['data'] = $this->db->get('popup_slider_image')->result();        
        return $result;
    }    
}
?>