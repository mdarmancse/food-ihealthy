...
var Datatable = function () {
	...
	return {
		...
		init: function (options) {
				...
				true,
				{
					...
					dataTable: {
						...
						oLanguage: {
							...
							sProcessing:
								'<img src="' +
								Metronic.getGlobalImgPath() +
								'loading-spinner-grey.gif"/><span>&nbsp;&nbsp;Loading...</span>',
							sLengthMenu:
								"<span class='seperator'>|</span>View _MENU_ records",
							sInfo:
								"<span class='seperator'>|</span>Found total _TOTAL_ records",
							sInfoEmpty: "No records found to show",
							sGroupActions: "_TOTAL_ records selected:  ",
							sAjaxRequestGeneralError:
							...
							oSettings.jqXHR = $.ajax({
								...
								error: function () {
									...
									Metronic.alert({
										...
										icon: "warning",
										message:
											tableOptions.dataTable.oLanguage.sAjaxRequestGeneralError,
										container: tableWrapper,
										place: "prepend",
									});
			...
			$(".dataTables_length select", tableWrapper).addClass(
			...
			table.on("click", ".filter-submit", function (e) {
				...
				$(
					'input.form-filter[type="checkbox"]:checked, input.form-filter[type="radio"]:checked',
					table
				).each(function () {
					the.addAjaxParam($(this).attr("name"), $(this).val());
				});
		...
		clearAjaxParams: function (name, value) {
			if (name) {
				var newAjaxParam = ajaxParams.filter((val) => {
					value != name;
				});
				ajaxParams = newAjaxParam;
			} else {
				ajaxParams = [];
			}
		},
